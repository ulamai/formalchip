...
formalchip report --run-dir ".formalchip/runs/${LAST_RUN}" --format json > ".formalchip/runs/${LAST_RUN}/report/summary.ci.json"

echo "Pilot completed: .formalchip/runs/${LAST_RUN}"
""",
        executable=True,
    )
...
- `ci/run-pilot.sh`: reproducible CI sequence (doctor -> synth -> run -> report).
...
formalchip run --config formalchip.toml
```
...
- `report/summary.json`, `report/summary.md`
